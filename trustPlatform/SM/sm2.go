...
import (
	...
	// "github.com/MonteCarloClub/Krypto/util"
)
...
func RawBytesToPublicKey(bytes []byte) (*PublicKey, error) {
	if len(bytes) != KeyBytes*2 {
		return nil, fmt.Errorf(fmt.Sprintf("Public key raw bytes length must be %d", KeyBytes*2))
	}
	publicKey := new(PublicKey)
	publicKey.Curve = sm2P256V1
...
func RawBytesToPrivateKey(bytes []byte) (*PrivateKey, error) {
	if len(bytes) != KeyBytes {
		return nil, fmt.Errorf(fmt.Sprintf("Private key raw bytes length must be %d", KeyBytes))
	}
	privateKey := new(PrivateKey)
	privateKey.Curve = sm2P256V1
...
func Verify(pub *PublicKey, userId []byte, src []byte, sign *Sm2Signature) (bool, error) {
	//r, s, err := UnmarshalSign(sign)
	//if err != nil {
	//	return false, err
	//}

	return VerifyByRS(pub, userId, src, sign.R, sign.S), nil
}
