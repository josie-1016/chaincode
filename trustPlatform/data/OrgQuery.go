...
// ===================================================================================
// 根据org user file name查询非fail状态的请求，如果有的话检查是否经过了3天，是的话将其置为fail，返回空
// ===================================================================================
func QueryThreholdFileApply(orgid string, uid string, fileName string, status ApplyStatus, stub shim.ChaincodeStubInterface) (apply *OrgApply, err error) {
	log.Println("query active threhold file apply: " + fileName)
	if status == Fail {
		return nil, ecode.Error(ecode.RequestErr, "can't query fail apply")
	}
	var queryString string
	if status == Pending {
		queryString = fmt.Sprintf("{\"selector\":{\"docType\":\"%s\",\"orgId\":\"%s\",\"fromUid\":\"%s\",\"attrName\":\"%s\",\"type\":2,\"status\":{\"$lt\":\"%d\"}}}",
			constant.OrgApply, orgid, uid, fileName, Success)
	} else {
		queryString = fmt.Sprintf("{\"selector\":{\"docType\":\"%s\",\"orgId\":\"%s\",\"fromUid\":\"%s\",\"attrName\":\"%s\",\"type\":2,\"status\":{\"$lt\":\"%d\"}}}",
			constant.OrgApply, orgid, uid, fileName, Success)
	}
...
// 根据org查询是否有对同一个组织生成门限
func QueryThresholdOrgApply(orgName string, status ApplyStatus, stub shim.ChaincodeStubInterface) (apply *OrgApply, err error) {
	log.Println("query active declare threshold org  apply: " + orgName)
	if status == Fail {
		return nil, ecode.Error(ecode.RequestErr, "can't query fail apply")
	}
	var queryString string
	if status == Pending {
		queryString = fmt.Sprintf("{\"selector\":{\"docType\":\"%s\",\"orgId\":\"%s\",\"type\":3,\"status\":{\"$lt\":\"%d\"}}}",
			constant.OrgApply, orgName, Success)
	} else {
		queryString = fmt.Sprintf("{\"selector\":{\"docType\":\"%s\",\"orgId\":\"%s\",\"type\":3,\"status\":%d}}",
			constant.OrgApply, orgName, status)
	}
	log.Println(queryString)
	resultsIterator, err := stub.GetQueryResult(queryString)
	if err != nil {
