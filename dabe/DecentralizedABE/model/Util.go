...
func ParsePolicyString(A *AccessStruct, s *string, startPos int, stopPos int) (*PolicyNode, int) {
	...
	for i <= stopPos {
		...
		if leftPos != -1 {
			...
			tmpPolicy, tmpID := ParsePolicyString(A, s, i+leftPos, rightPos) //继续递归（C OR D）
